...
public class FireworkBotan extends Firework implements Runnable{
	...
	private static final String TAG = "FireworkBotan" ;
	...
	private float[][] flashTiming = null; 
	private static final int FLASH_HALF_CYCLE = 300 ;
	...
	public FireworkBotan(long seed, int lifeMilliTime,  int drawQuality,float v, float startX, float startY, float startZ, 
	...
	protected void initSeeds(long seed){
		...
		randomFlash = new Random(rand.nextLong()) ;
	}
	...
	public void run(){
		...
		for(int i=0 ; i<ringSizes.length ; i++){
			for(int j=0; j<ringSizes[i] ; j++){
				if(randmoStarFlashEnable){
                    flashTiming[i][j] = randomFlash.nextFloat() ; 
				}else{
                    flashTiming[i][j] = 1.0f;
				}
			}
		}
	...
	public void drawFireworks(int textureId, int time){
		...
		for(int i=0 ; i<ringSizes.length ; i++){
			...
			for(int j=0 ; j<size ; j++){ 
				...
				float b = rgba[2] ;
				float a = rgba[3] ;
				int cycle = -2*((time+(int)(FLASH_HALF_CYCLE*flashTiming[i][j])) % FLASH_HALF_CYCLE) + FLASH_HALF_CYCLE ; 
				float flashSwitch = (cycle<0 && time>=startFlashTime) ? 0.8f: 1.0f;
				r *= flashSwitch ;
				g *= flashSwitch ;
				b *= flashSwitch ;
				a *= flashSwitch ;
...
*/
	...
	private float[] nextVertexForScene(int scene){
		...
		float preBuff_xyz[][] = new float[ringSizes.length][] ;
		float fallDistance = fallDistanceForScene[scene] ;
		int bufCount = 0 ;
		int starIndex = 0 ;
		for(int i=0 ; i<ringSizes.length ;i++){
			preBuff_xyz[i] = new float[ringSizes[i]*3] ;
			for(int j=0 ; j<ringSizes[i] ; j++){
				float r = radiusesForScene[scene][starIndex] ;
				...
				preBuff_xyz[i][j*3] = x + startX ;
				preBuff_xyz[i][j*3+1] = y + startY - fallDistance;
				preBuff_xyz[i][j*3+2] = z + startZ ;
				bufCount+=3 ;
				starIndex++ ;
			}
		}
		int i=0 ; 
		float[] preBuff = new float[bufCount] ;
		for(float[] tmpA : preBuff_xyz){
			for(float tmp : tmpA){
				preBuff[i] = tmp ;
				i++ ;
			}
